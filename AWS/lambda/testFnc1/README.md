...
```bash
testFnc1$ sam local invoke testFnc --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
testFnc1$ sam local start-api
testFnc1$ curl http://localhost:3000/
```
...
```bash
testFnc1$ sam logs -n testFnc --stack-name testFnc1 --tail
```
...
```bash
aws cloudformation delete-stack --stack-name testFnc1
```
